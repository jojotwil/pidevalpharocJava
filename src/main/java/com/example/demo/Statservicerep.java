...
public class Statservicerep implements Initializable {
...
// Définissez la taille de la fenêtre sur les dimensions de l'écran
    ...
    @FXML
    private PieChart pieChartEvRes;

    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        DemandeservServices demandeservServices = new DemandeservServices();
        pieChartEvRes.setData(demandeservServices.getDemandeServStatsByCategorie());
    }
    @FXML
    void backstat(ActionEvent event) throws  Exception {
        try {
            Parent adminRoot = FXMLLoader.load(getClass().getResource("/Gestiondemandeservadmin.fxml"));
    ...
    @FXML
    void quitstat(ActionEvent event) {
        // Fermer la fenêtre principale de l'application
        Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
        stage.close();
        // Arrêter complètement l'application JavaFX
        Platform.exit();
        // Terminer le processus Java
        System.exit(0);
    }
}
