...
public class PartenaireBack implements Initializable {
...
// Définissez la taille de la fenêtre sur les dimensions de l'écran
    ...
    void showDemandeServicerep() {
        ...
        try {
            ...
            List<Demandeserv> demandes = demandeservServices.getDemandesbyServiceId(this.serviceId );
    ...
    @FXML
    void refuserbtn(ActionEvent event) throws IOException {
        // Récupérer l'élément sélectionné dans le TableView
        Demandeserv selectedDemande = tvdemandeservice.getSelectionModel().getSelectedItem();
        ...
        selectedDemande.setStatut("refuser");

        // Mettre à jour l'élément sélectionné dans la base de données
        DemandeservServices demandeservServices = new DemandeservServices();
        demandeservServices.updateEntity(selectedDemande, selectedDemande.getId());

        // Envoyer un SMS à l'utilisateur
        String message = "Votre demande de service "+ selectedDemande.getNom()+" a été refusée.";

      /*  String phoneNumber = String.valueOf(selectedDemande.getDuree());
        selectedDemande.setDuree(Float.parseFloat(phoneNumber));*/
        String phoneNumber ="97621631";
        SendSms sendSms = new SendSms();
        //   if (sendSmsService != null) {sendSmsService.sendSms(message, duree.getText());}
        sendSms.sendSms(message, phoneNumber);
    ...
    void acceptbtn(ActionEvent event) throws IOException {
        ...
        // Mettre à jour le statut de la demande de service
        selectedDemande.setStatut("accepter");
        ...
        // Envoyer un SMS à l'utilisateur
        String message = "Votre demande de service "+ selectedDemande.getNom()+"a été acceptée.";
    /*    String phoneNumber = "+216" + String.valueOf(selectedDemande.getDuree());
        selectedDemande.setDuree(Float.parseFloat(phoneNumber)); // Format du numéro de téléphone*/
        String phoneNumber ="97621631";
        SendSms sendSms = new SendSms();
        //   if (sendSmsService != null) {sendSmsService.sendSms(message, duree.getText());}
        sendSms.sendSms(message, phoneNumber);
    ...
    public void setServiceId(String serviceId) {
        searchField.setText(serviceId); // Mettre l'ID du service dans le champ de recherche
    }

    @FXML
    void searchById(ActionEvent event) {
        DemandeservServices bs = new DemandeservServices();
        try {
            String searchText = searchField.getText().toLowerCase(); // Récupérer le texte de recherche et le convertir en minuscules

            // Récupérer la colonne spécifique de TableView
            TableColumn<Demandeserv, String> colidservice = (TableColumn<Demandeserv, String>) tvdemandeservice.getColumns().get(0); // Supposons que c'est la première colonne

            // Définir la valeur de la cellule pour la propriété représentant les données à filtrer
            colidservice.setCellValueFactory(new PropertyValueFactory<>("titre")); // Supposons que la propriété dans votre classe Demandeserv représentant les données dans la colonne est appelée "titre"

            // Obtenir toutes les données de la TableView
            List<Demandeserv> allData = bs.getAllData();

            // Créer une liste observable à partir des données de la TableView
            ObservableList<Demandeserv> observableList = FXCollections.observableList(allData);

            // Filtrer les éléments dans la TableView en fonction du texte de recherche
            ObservableList<Demandeserv> filteredList = observableList.filtered(item ->
                    String.valueOf(item.getServicerep_id()).toLowerCase().contains(searchText)
            );

            // Mettre à jour la TableView avec la liste filtrée
            tvdemandeservice.setItems(filteredList);
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        searchField.textProperty().addListener((observable, oldValue, newValue) -> {
            // Appelez la méthode searchById lorsque le texte du champ de recherche change
            searchById(new ActionEvent());
        });
